...
import 'dart:math';
import 'dart:async';
import 'platform_equalizer.dart';
import 'package:shared_preferences/shared_preferences.dart';
import '../model/local_song_model.dart';
import '../utils/safe_audio_query.dart';
...
class AudioController {
  ...
  void _setupAudioPlayer() {
    ...
    // Subscribe to changes from EqualizerProvider isn't possible here (no BuildContext)
    // Instead, consumers will trigger applySettingsFromProvider when provider changes via main UI rebuild.
  }
  ...
  Future<void> applySettingsFromProvider(dynamic provider) async {
    try {
      // access provider dynamically to avoid tight coupling
  ...
  Future<bool> playSong(int index, {bool fromPlaylist = false}) async {
    ...
    try {
      ...
      if (_audioHandler != null && _isInitialized) {
        try {
          // For online songs, use the image URL directly; for local songs, construct album art URI
          Uri? artUri;
          String? albumName;

          if (song.albumArt.isNotEmpty) {
            if (song.albumArt.startsWith('http')) {
              // Online song - use the URL directly
              artUri = Uri.parse(song.albumArt);
              albumName = 'Online';
            } else {
              // Local song - use the albumArt field which contains album name
              albumName = song.albumArt;

              // Try to get album art URI from MediaStore
              // For local songs, we'll construct a content URI
              try {
                // Query to get the song's album ID
                ...
                // Get album ID from the matched song
      ...
      try {
        ...
        if (uri.scheme.isEmpty && !isOnlineSong) {
          ...
          // For local files, ensure proper file URI
        ...
        // Apply persisted settings (volume/speed/pitch/gains) just before play to ensure they take effect
  ...
  Future<bool> playOnlineUrl(String url, {String? title, String? artist, String? imageUrl, int? duration}) async {
    try {
      ...
      // Create a LocalSongModel for the online song to enable mini player
      final onlineSong = LocalSongModel(
        id: url.hashCode, // Use URL hash as unique ID
      ...
      // Update current online song and state
      currentOnlineSong.value = onlineSong;
      _isPlayingFromPlaylist = false;

      // Add to a temporary online playlist for proper indexing
...
// Add this inside AudioController
  ...
  Future<bool> playFromPlaylist(LocalSongModel song, {List<LocalSongModel>? playlist}) async {
    if (playlist != null && playlist.isNotEmpty) {
      ...
      // Find song by URI instead of ID (since online songs have dynamic IDs)
      ...
      // Add to existing playlist or create new one
  ...
  // Keys must match EqualizerProvider persistence keys
